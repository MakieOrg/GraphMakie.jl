...
graphplot!(
    g; layout = SquareGrid(), nlabels, nlabels_align, nlabels_distance = 30,
...
p[:nlabels][] = ["×" for i in 1:nv(g)]
p[:nlabels_color][] = :red
p[:nlabels_offset][] = [Point2(0.1 * cos(-2π / 9 * i), 0.1 * sin(-2π / 9 * i)) for i in 1:nv(g)]
@save_reference fig
...
fig, ax, p = graphplot(
    g; layout = SquareGrid(), elabels, elabels_align,
    elabels_color = :red
)
hidedecorations!(ax)
@save_reference fig
...
p[:node_pos][] = Point2f.([(1.0, -0.5), (-1.0, 0.0), (-1.0, -1.0)])
hidedecorations!(ax)
@save_reference fig
